        ...
        <Card style={{ backgroundColor: 'var(--amber-4)'}}>
          <Inset clip="padding-box">
            ...
            {kid.outcomeOptions.map(option => 
              <label key={option.id} style={giveStyle(option.outcome)} htmlFor={option.outcome}>
                { option.outcome } 
                <input type='checkbox' name='outcome' id={option.outcome} value={option.outcome} style={{visibility: 'hidden'}} {...register('outcomes')} />
              </label>
            )}
            </Flex>
          </Inset>
